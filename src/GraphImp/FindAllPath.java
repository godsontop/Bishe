...
public class FindAllPath {
    ...
    public int getNextNode(LinJieJuZhen graph,int x,int y){
        int next_node=-1;
        LinJieJuZhen.EdgeNode edge=graph.vexsarray[x].firstEdge;
        if(null!=edge&&y==-1){
            int n=edge.adjvex;
            //元素还不在stack中
            if(!states.get(n))
    ...
    public void visit(LinJieJuZhen graph,int x,int y){
        //初始化所有节点在stack中的情况
